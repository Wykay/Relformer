...
class MPLUG(nn.Module):
    def __init__(self,                 
        ...
        self.visual_encoder, _ = initialize_clip(config)                                                                                # load cilp pretrained vit or mPLUG pretrained vit(download from github page)
        self.text_encoder = BertModel.from_pretrained('bert-base-uncased', config=self.config_encoder, add_pooling_layer=False)      # text_encoder & text_decoder(12 layers) : bert-base-uncased
        self.fusion_encoder = FusionModel.from_pretrained('bert-base-uncased', config=self.config_fusion, add_pooling_layer=False)   # self.config_encoder/config_fusion/config_decoder = BertConfig.from_json_file(config['bert_config'])   configs/config_bert.json
        self.text_decoder = BertPrefixModel.from_pretrained('bert-base-uncased', config=self.config_decoder)    
