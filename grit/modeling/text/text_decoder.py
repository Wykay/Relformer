...
class BertEncoderAsDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,                                                                          #  tgt: text_embeddings(also target, since we use self-attention)     memory: projected img features
                tgt_mask=None,
        ...
        hidden_states = torch.cat((memory, tgt), dim=1)                                                 # concat img_features and text_embeddings at channel dimension
        num_tgt = tgt.shape[1]
        ...
        if encoder_history_states is None:
            result = self.encoder(                                                              # use encoder so there is only self-attention and ffn
...
class TransformerDecoderTextualHead(TextualHead):
    ...
    def forward(
        ...
        # project object feature to texual feature size
        ...
        trans_out = self.transformer(                                           # BertEncoderAsDecoder
            text_embeddings,
...
class GRiTTextDecoder(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            output_logits = self.textual(           # TransformerDecoderTextualHead
                object_features,
                caption_token_input,
            )
...
class SmoothLabelCrossEntropyLoss(nn.Module):
    ...
    def forward(self, feature, target):
        ...
        one_hot = torch.zeros_like(feature).scatter(1, target.view(-1, 1), 1)     # target(-1,1)  shape,  filled with 1 
        one_hot = one_hot * (1 - eps) + (1 - one_hot) * eps / (n_class - 1)       # smaller weight (0~1)
        log_prb = self.log_soft(feature)                                                # LogSoftmax    log(0~1)
        loss = self.kl(log_prb, one_hot)                                                # this loss expects the argument input(log_prb) in the log-space   loss_pointwise = (target * (target.log() - input)).mean()
