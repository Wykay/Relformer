...
class ViT(Backbone):
    ...
    def forward(self, x):
        x = self.patch_embed(x)                                                             # x:  6, 3, 1024, 1024    B C H W -> B H W C    6, 64, 64, 768
        if self.pos_embed is not None:
            x = x + get_abs_pos(
...
class ViT_FPN(Backbone):
    ...
    def forward(self, x):
        vit_output_featuremap = self.bottom_up(x)                                                            # 6, 768, 64, 64
        ...
        fpn_out = {f: res for f, res in zip(self._out_features, results)}           # p3,p4,p5,p6,p7          p3: 6, 256, 128, 128   p4: 6, 256, 64, 64   p5: 6, 256, 32, 32   p6: 6, 256, 16, 16
