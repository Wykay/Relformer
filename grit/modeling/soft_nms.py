...
def _soft_nms(
   ...
   # scale down the scores of largly overlapped boxes
    while scores.numel() > 0:
         ...
         if method == "linear":
            ...
            # decay[decay_mask] = 1 - ious[decay_mask]                                               # the larger the iou is, the greater the decay is, 1 - iou, the smaller score the similar box have
            decay[decay_mask] = (1 - ious[decay_mask])/ ((1-linear_threshold)*10)                                         
         ...
         keep[top_idx] = False                      # throw out the first box, i.e. the one with the largest confidence score
