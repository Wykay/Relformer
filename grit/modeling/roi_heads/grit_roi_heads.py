...
from ..text.text_decoder import TransformerDecoderTextualHead, GRiTTextDecoder, AutoRegressiveBeamSearch                    # import text decoder
...
class GRiTROIHeadsAndTextDecoder(CascadeROIHeads):
    @configurable           # 用from_config，config文件中的参数去初始化模型，用config文件中的参数去执行init
    def __init__(
        self,
        *,
        ...
        object_feat_pooler=None,                # OBJECT_FEAT_POOLER_RES = 14
        ...
        **kwargs,
        ...
        tokenizer = AutoTokenizer.from_pretrained("google-bert/bert-base-cased", do_lower_case=True)          # , local_files_only=True
    ...
    def check_if_all_background(self, proposals, targets, stage):
        ...
        for proposals_per_image in proposals:
            if not (proposals_per_image.gt_classes == self.num_classes).all():                                      # 0 stands for foreground
                all_background = False

            # if len(proposals_per_image.gt_classes[proposals_per_image.gt_classes<1]) < 10:
            #     all_background = True
        ...
        if all_background:
            ...
            # for i in range(0,len(proposals[0].proposal_boxes)):                                                                     # `Instances` object is not iterable!
            #     if i < 10:
            #         proposals[0].proposal_boxes.tensor[i, :] = targets[0].gt_boxes.tensor[i, :]
            #         proposals[0].gt_boxes.tensor[i, :] = targets[0].gt_boxes.tensor[i, :]
            #         proposals[0].objectness_logits[i] = math.log((1.0 - 1e-10) / (1 - (1.0 - 1e-10)))
            #         proposals[0].gt_classes[i] = targets[0].gt_classes[i]
            #         proposals[0].gt_object_descriptions.data[i] = targets[0].gt_object_descriptions.data[i]
            #         if 'foreground' in proposals[0].get_fields().keys():
            #             proposals[0].foreground[i] = 1
    ...
    def _forward_box(self, features, proposals, targets=None, task="ObjectDet"):
        ...
        if self.training:
            ...
            if len(object_descriptions) > 0:
                ...
                if object_features.shape[0] != len(object_descriptions):
                    print('mismatch')

                text_decoder_loss = self.text_decoder(text_decoder_inputs)                                              # object_features: [49, 196, 256]    object_descriptions: 49
            else:
    ...
    def fast_rcnn_inference_GRiT(
        ...
        if descriptions == None:
            ...
            result_per_image = [
                self.fast_rcnn_inference_single_image_GRiT(
                    boxes_per_image, scores_per_image, logits_per_image, image_shape,
                    score_thresh, nms_thresh, topk_per_image, soft_nms_enabled,descriptions
                )
                for scores_per_image, boxes_per_image, image_shape, logits_per_image, descriptions \
                in zip(scores, boxes, image_shapes, logits, descriptions)
            ]            
    ...
    def fast_rcnn_inference_single_image_GRiT(
        ...
        # # second round of nms
        # keep = batched_nms(boxes, scores, filter_inds[:, 1], 0.6)
        # if topk_per_image >= 0:
        #     keep = keep[:topk_per_image]
        # boxes, scores, filter_inds = boxes[keep], scores[keep], filter_inds[keep]
        ...
        # if image captioning
        # boxes[0] = torch.Tensor([0,0,image_shape[1],image_shape[0]])
        # boxes = torch.stack([boxes[0]],0)
        # scores = torch.stack([scores[0]],0)
        # filter_inds = torch.stack([filter_inds[0]],0)
        # logits = torch.stack([logits[0]],0)
