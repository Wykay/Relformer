...
class MPLUG(nn.Module):
    def __init__(self,                 
        ...
        self.text_encoder = BertModel.from_pretrained(config['text_encoder'], config=self.config_encoder, add_pooling_layer=False)      # text_encoder & text_decoder(12 layers) : bert-base-uncased
        self.fusion_encoder = FusionModel.from_pretrained(config['text_encoder'], config=self.config_fusion, add_pooling_layer=False)   # self.config_encoder/config_fusion/config_decoder = BertConfig.from_json_file(config['bert_config'])   configs/config_bert.json
