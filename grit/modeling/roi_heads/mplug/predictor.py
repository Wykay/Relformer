...
class TextGenerator(object):
    ...
    def translate_batch_scst(self, encoder_inputs, do_sample=False, out_size=1, hs=None):
        return self._fast_translate_batch(encoder_inputs, self.max_length, min_length=self.min_length, do_sample=do_sample,out_size=out_size,hs=hs)
    ...
    def _fast_translate_batch(self,
        ...
        if len(encoder_inputs) == 3:
            ...
            src_features, padding_mask = encoder_inputs                                             # inference
            input_ids = None
        ...
        if input_ids is not None:
            ...
            alive_seq = torch.full(
                ...
                self.start_token,
                dtype=torch.long,                   
                device=device)                                                                  # 260,1   bos

        # Give full probability to the first beam on the first step.
        topk_log_probs = (
            torch.tensor([0.0] + [float("-inf")] * (beam_size - 1),
                         device=device).repeat(batch_size))                                     # 260,1      for each sentence, the first beam got 0, while the rest fours got -inf.   so the first beam on the first step got probability=1  log(0)=1

        # Structure that holds finished hypotheses.
        hypotheses = [[] for _ in range(batch_size)]  # noqa: F812                              # 52,1   
        ...
        for step in range(max_length):
            ...
            # debug for whole2local
            if step == 0 and hs != None:
            ...
            dec_feat_seq = dec_feat_seq.logits[:, -1, :]                                # dec_feat_seq.logits shape     260,30522                                                   decode the next token and get its logit distribution
            vocab_size = dec_feat_seq.size(-1)
            log_probs = torch.log(torch.softmax(dec_feat_seq.view(-1, vocab_size), dim=-1))                     # softmax on logits                                                 perform softmax and log(p) on it
            if step < min_length:
               log_probs[:, self.end_token] = -1e20                                                             # if current length(i.e. step) < min_length,  make the log(p) of <eos> to be a very small number( so we won't stop here as finished)
            alpha = self.alpha                                                                                  # global_scorer.alpha   0.6
            if do_sample:
                length_penalty = 1.0
            else:
                length_penalty = ((5.0 + (step + 1)) / 6.0) ** alpha                                            # the longer the sentence is, the greater the penalty is.
            ...
            if do_sample:
                ...
                # 相加起来相当于考虑了前文，自回归的方式
                log_probs += topk_log_probs.view(-1).unsqueeze(1)                   # 所以这个logprobs 是每个token的logprob 相加的结果，相当于概率相乘   # 260 * 30522  log_probs correspod to every beam     topk_log_probs 表示当前 step 每个 beam 的最大概率  260，1   将它与 logprobs 加起来得到当前 step 所有句子的 logprob
                curr_scores = log_probs / length_penalty

                curr_scores = curr_scores.reshape(-1, beam_size * vocab_size)                               # batch_size*beam_size, 30522 ->  batch_size, beam_size*30522    perform topk on it
                topk_scores, topk_ids = curr_scores.topk(beam_size, dim=-1)                                 # got top beam_size prediction for each sentence     52,5        batch_size,beam_size
                topk_log_probs = topk_scores * length_penalty

            # Resolve beam origin and true word ids.
            topk_beam_index = torch.div(topk_ids, vocab_size, rounding_mode='trunc')                        # 整除                  52,5   第一个 step 全 0 
            topk_ids = topk_ids.fmod(vocab_size)                                                            # 计算余数              52,5   每个 token 对应 vocabulary 中的 id，即输出结果

            # Map beam_index to batch_index in the flat representation.
            batch_index = (
                    topk_beam_index
                    + beam_offset[:topk_beam_index.size(0)].unsqueeze(1))
            select_indices = batch_index.view(-1)                                                           # 本 batch 数据的 beam 束

            # Append last prediction.
            alive_seq = torch.cat(
                [alive_seq.index_select(0, select_indices),
                 topk_ids.view(-1, 1)], -1)                                                                 # 把当前 step 的 52,5 个结果插入到 alive_seq 里面           alive_seq:  260,1  ->  260,2  第一个 token 都是 bos,  现在插入第二个生成的结果 

            is_finished = topk_ids.eq(self.end_token)                                           #   [batch_size,beam_size]              看当前 step 的 所有 beam 里面有没有 eos，若有则该句子生成结束
            if step + 1 == max_length:
                is_finished.fill_(1) #self.end_token)
            # End condition is top beam is finished.
            end_condition = is_finished[:, 0].eq(1) #self.end_token)                                        # 当每个句子的 top beam(第一个)  都 finished 时，结束 beam search    52,1
            # Save finished hypotheses.
            if is_finished.any():                                                                           # 若任意 beam 生成结束，  将它的结果暂时保存下来
                predictions = alive_seq.view(-1, beam_size, alive_seq.size(-1))                             # batch_size,5,step
                for i in range(is_finished.size(0)):                                                        # batch_size
                    b = batch_offset[i]
                    if end_condition[i]:
                    ...
                    for j in finished_hyp:
                        hypotheses[b].append((
                            topk_scores[i, j],                                                               # 这个相当于最后的输出句子的 score    还是蛮低的    是经过了 logp 连乘的结果
                            predictions[i, j, 0:]))
                    # If the batch reached the end, save the n_best hypotheses.
                    if end_condition[i]:
                        best_hyp = sorted(
                            hypotheses[b], key=lambda x: x[0], reverse=True)                                # len : beam_size     根据 score 排序 每个 beam   每个元素由 score, sentence 组成
                        ...
                        for each in best_hyp[:beam_size]:
                            score, pred = each
                            results["scores"][b].append(score)                                                          # len(results["scores"]) = batch_size
                            results["predictions"][b].append(pred)                                                      # len(results["predictions"]) = batch_size

                            # plt.figure(figsize=(8, 4))
                            # plt.text(0.5, 0.5, pred, fontsize=12, ha='center', va='center')
                            # plt.axis('off')  # 关闭坐标轴
                            # plt.show()

                non_finished = end_condition.eq(0).nonzero().view(-1)
                # If all sentences are translated, no need to go further.
                if len(non_finished) == 0:                                                                              # break if finish all beams
                    break
            ...
            # Reorder states.                                                                                           # 恢复下一个 step 的状态
            select_indices = batch_index.view(-1)                                                                       # 260
            src_features = src_features.index_select(0, select_indices)                                                 # 260,196,768             选择对应每个sentence 的 beam 号， copy???                                      
        ...
        for i,each in enumerate(results["scores"]):
            score = each[:out_size][0] / torch.Tensor([len(pred_ids[i][0])-1]).to(score.device)
            # score = each[:out_size][0].to(score.device)
            scores.append( [score] )
